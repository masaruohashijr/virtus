...
func registrarProdutosComponentesTodos(currentUser mdl.User) {
	sqlStatement := "INSERT INTO virtus.produtos_componentes ( " +
		...
		" SELECT " +
		" NEXT VALUE FOR virtus.id_produtos_componentes_seq, " +
		" ce.id_entidade, " +
		" a.id_ciclo, " +
		" a.id_pilar, " +
		" b.id_componente, " +
		...
		" LEFT JOIN virtus.PILARES_CICLOS a ON (a.id_ciclo = ce.id_ciclo) " +
		" LEFT JOIN virtus.COMPONENTES_PILARES b ON (a.id_pilar = b.id_pilar) " +
		" LEFT JOIN virtus.E																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																					LEMENTOS_COMPONENTES c ON (b.id_componente = c.id_componente) " +
		" WHERE  " +
		" NOT EXISTS " +
		"  (SELECT 1 " +
		...
		" GROUP BY a.id_ciclo,a.id_pilar,b.id_componente"
	log.Println(sqlStatement)
...
func DeletePilarHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" && sec.IsAuthenticated(w, r) {
		...
		sqlStatement := "DELETE FROM virtus.produtos_ciclos WHERE id_pilar=?"
		...
		sqlStatement = "DELETE FROM virtus.produtos_pilares WHERE id_pilar=?"
		deleteForm, _ = Db.Prepare(sqlStatement)
		_, err = deleteForm.Exec(id)
		if err != nil && strings.Contains(err.Error(), "violates foreign key") {
			http.Redirect(w, r, route.PilaresRoute+"?errMsg="+errMsg, 301)
		...
		sqlStatement = "DELETE FROM virtus.produtos_componentes WHERE id_pilar=?"
		deleteForm, _ = Db.Prepare(sqlStatement)
		_, err = deleteForm.Exec(id)
		if err != nil && strings.Contains(err.Error(), "violates foreign key") {
			http.Redirect(w, r, route.PilaresRoute+"?errMsg="+errMsg, 301)
